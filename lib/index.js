...
async function openCompletion(auth, params, signal, transport) {
	...
	for (let attempt = 0; attempt < 2; attempt++) {
		const sessionId = (await leaseSession(auth, signal, transport, params.sessionReuseTurns ?? 20, params.onDeleteSession)).sessionId;
		let resp;
		try {
...
function serializePrompt(options) {
	...
	return head + separator + truncateMiddle(transcript, budget, .7);
}
