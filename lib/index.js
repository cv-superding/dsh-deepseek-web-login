...
const DEFAULT_MIN_REQUEST_INTERVAL_MS = 3e3;
/** 间隔可选的推荐档位（设置页的快捷按钮用）。 */
...
function createRequestGate(options = {}) {
	...
	async function acquire(label = "call") {
		...
		if (minIntervalMs > 0 && hasFinished) {
			...
			if (waitMs > 0) {
				logger?.info?.(`deepseek-web: 距上次请求不足 ${minIntervalMs}ms，等 ${Math.round(waitMs)}ms 再发「${label}」（防账号级限流）`);
				await sleep(waitMs);
			}
		}
		running += 1;
	...
	function configure(next) {
		...
		logger?.info?.(`deepseek-web: 请求节流设置已更新 —— ${allowConcurrent ? "允许并发（不推荐）" : "串行"} · 间隔 ${minIntervalMs}ms`);
		return settings();
	}
	return {
		acquire,
...
function createAdapter(deps) {
	...
	const gate = deps.gate ?? createRequestGate({
		...
		minIntervalMs: deps.config.minRequestIntervalMs ?? 3e3,
		logger
	});
	const adapter = {
...
function apply(ctx, config = {}) {
	...
	const gate = createRequestGate({
		...
		minIntervalMs: savedGate?.minRequestIntervalMs ?? config.minRequestIntervalMs ?? 3e3,
		logger
	});
	const adapterConfig = {
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (req.method === "POST" && route === "/gate") {
					...
					if (body.minRequestIntervalMs !== void 0) {
						const ms = Number(body.minRequestIntervalMs);
						if (!Number.isFinite(ms)) {
							sendJson(res, 400, {
								ok: false,
								error: "minRequestIntervalMs 必须是数字"
							});
							return;
						}
				...
				if (req.method === "GET" && (route === "/status" || route === "/")) {
					...
					sendJson(res, 200, {
						...
						config: {
							...
							minRequestIntervalMs: adapterConfig.minRequestIntervalMs ?? 3e3
						}
					});
					return;
				}
