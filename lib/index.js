	...
	async function* streamImpl(options) {
		...
		try {
			...
			for (;;) {
				...
				logger?.info?.(`deepseek-web: 第 ${rounds + 1} 轮流结束：[本轮 ${roundChars} 字 / 累计 ${partial.length} 字 / 耗时 ${Date.now() - roundStartedAt}ms] finish=${finishReason ?? "(无 FINISHED → 服务端截断)"}${midSentence ? "，尾部是句中" : ""}${roundChars === 0 && rounds === 0 ? "（⚠️ 本轮正文 0 字 —— 内容可能全在思考通道）" : ""}`);
