			...
			for (;;) {
				...
				try {
					for await (const event of runStream(auth, {
						...
						if (event.kind === "finish") event.reason;
					}
				} catch (error) {
					if (rounds > 0) {
				...
				if (tail.rejected) {
					...
					logger?.warn?.(`deepseek-web: 工具调用${tail.rejected.mode === "xml" ? "（XML）" : ""}解析失败[${tail.rejected.reason ?? "unparsable"}]，已丢弃 ${tail.rejected.raw.length} 字符（完整原文见 ~/.dsh/deepseek-web/rejected.jsonl）：` + tail.rejected.raw.slice(0, 2e3));
					if (rounds === 0) {
				...
				if (!(roundError === void 0 && deps.config.autoContinue !== false && rounds < maxRounds && toolCallCount === 0 && !options?.signal?.aborted && partial.length > 0 && looksMidSentence(partial))) break;
				rounds += 1;
