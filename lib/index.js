...
const DEFAULT_MAX_PROMPT_CHARS = 15e5;
...
function createRequestGate(options = {}) {
	...
	let maxPromptChars = clampMaxPromptChars(options.maxPromptChars ?? 15e5);
	...
	*/
	async function uploadRequestImages(auth, messages, signal) {
		...
		return failures.length > 0 ? {
			ids,
			notice: imageNotice(failures.length, failures[0])
		} : { ids };
	}
	/**
	* 图片没能送进模型时的用户可见告知。
...
function apply(ctx, config = {}) {
	...
	const gate = createRequestGate({
		...
		maxPromptChars: savedGate?.maxPromptChars ?? config.maxPromptChars ?? 15e5,
