...
function upsertAccount(auth, patch = {}) {
	...
	const existing = (serverId ? all.find((item) => item.serverId && item.serverId === serverId) : void 0) ?? all.find((item) => item.token === auth.token);
...
function importAccounts(payload) {
	...
	for (const raw of list) {
		...
		const matched = (candidate.serverId ? before.find((item) => item.serverId && item.serverId === candidate.serverId) : void 0) ?? before.find((item) => item.token === candidate.token);
		if (matched) {
			updateAccount(matched.id, {
				...candidate,
				id: matched.id,
	...
	async function acquire(label = "call", signal) {
		...
		try {
			...
			if (hasFinished && (needsBreak || maxIntervalMs > 0)) {
				...
				if (needsBreak) {
					consecutive = 0;
					logger?.info?.(`deepseek-web: 已连续 ${longRunThreshold} 次请求 —— 长休 ${Math.round(gap / 1e3)}s 再继续（长任务保护：连续跑比间隔小更像脚本）`);
				}
				if (waitMs > 0) {
...
async function loadWasmModule(wasmUrl) {
	...
	const promise = (async () => {
		...
		if (Number.isFinite(declared) && declared > MAX_WASM_BYTES) throw new Error(`PoW WASM 体积异常（${declared} 字节，上限 ${MAX_WASM_BYTES}）：拒绝加载`);
		const bytes = new Uint8Array(await resp.arrayBuffer());
		if (bytes.byteLength > MAX_WASM_BYTES) throw new Error(`PoW WASM 体积异常（${bytes.byteLength} 字节，上限 ${MAX_WASM_BYTES}）：拒绝加载`);
...
function classifyAuthEnvelope(json) {
	if (!json || typeof json !== "object" || Array.isArray(json)) return {
		ok: false,
		error: "users/current 响应不是 JSON 对象（可能是反爬页面或网关拦截）"
	};
	...
	if (json.data === void 0 && json.code === void 0) return {
		ok: false,
		error: "users/current 响应既无 data 也无 code（形状不符）"
	};
	return { ok: true };
}
...
async function leaseSession(auth, signal, transport, maxTurns) {
	...
	const retired = reuseSlot && reuseSlot.key === key ? reuseSlot.sessionId : void 0;
...
async function openCompletion(auth, params, signal, transport) {
	...
	for (let attempt = 0; attempt < 2; attempt++) {
		const lease = await leaseSession(auth, signal, transport, params.sessionReuseTurns ?? 20);
...
async function* streamWebCompletion(auth, params, transport = defaultTransport) {
	...
	try {
		...
		if ((params.sessionReuseTurns ?? 20) === 0) params.onDeleteSession?.(sessionId);
	}
}
//#endregion
	...
	*/
	...
	async function* streamImpl(options) {
				...
				try {
					for await (const event of runStream(auth, {
						...
						if (event.kind === "finish") {
							...
							if (typeof event.totalTokens === "number" && Number.isFinite(event.totalTokens)) reportedTokens += event.totalTokens;
						}
					}
				} catch (error) {
					if (rounds > 0) {
		...
		yield {
			type: "usage",
			usage: {
				inputTokens: reportedTokens > 0 ? Math.max(0, reportedTokens - outputTokens) : estimateTokens(prompt),
				outputTokens,
...
async function captureFromPartition(logger) {
	...
	for (const token of candidates) {
		...
		const check = await validateAuth(auth);
		if (check.ok) {
			const commit = commitCapturedAuth({
				...auth,
				...check.user ? { user: {
					...auth.user,
					...check.user
...
*/
function summarizeLedger(hours = 24) {
	const now = Date.now();
	const entries = readEntries(now - hours * 36e5);
	const failures = {};
	let succeeded = 0;
...
* 兜底常量与 package.json 的一致性由 `tests/check-version.mjs` 守着，不会漂。
*/
/** 与 package.json 保持一致的兜底版本（由测试保证不会漂）。 */
const FALLBACK_VERSION = "0.1.44";
let cached;
/** 本插件版本（如 `0.1.26`）。 */
