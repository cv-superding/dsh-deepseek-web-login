...
function stripStrayToolMarkup(text) {
	...
	return text.replace(new RegExp(`<\\/\\s*(?:[|｜]+\\s*DSML\\s*[|｜]+\\s*)?(?:dsml-)?(?:${WRAPPER_NAMES}|invoke)\\s*>`, "gi"), "").replace(/<\/\s+(?:tool_calls?|function_calls|calls|invoke)\s*>/gi, "").replace(/(^|\n)[ \t]*(?:in)?voke\s*>\s*(?=\n|$)/gi, "$1");
}
/**
* 流式工具调用过滤器。
