...
async function openCompletion(auth, params, signal, transport) {
	...
	for (let attempt = 0; attempt < 2; attempt++) {
		...
		const failure = biz ? new AdapterLlmError(muted ? mutedMessage(untilMs) : busy ? "网页版限流：同一账号同时只能生成一条消息，稍后自动重试" : bizErrorMessage(biz.code, biz.msg), muted || busy ? "RATE_LIMIT" : isInvalidSessionError(biz) ? "TRANSPORT" : isInvalidRefFileError(biz) ? "INVALID_REF_FILE" : bizErrorCode(biz.code), {
			status: resp.status,
