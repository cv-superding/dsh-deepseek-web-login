...
async function openCompletion(auth, params, signal, transport) {
	...
	for (let attempt = 0; attempt < 2; attempt++) {
		...
		const failure = biz ? new AdapterLlmError(muted ? mutedMessage(untilMs) : bizErrorMessage(biz.code, biz.msg), muted ? "RATE_LIMIT" : isInvalidSessionError(biz) ? "TRANSPORT" : bizErrorCode(biz.code), {
			status: resp.status,
...
function* structuralRepairCandidates(text) {
	...
	const fixed = scanned.chunks.map((chunk, index) => deficits[index] > 0 ? `${chunk.replace(/[\s,]+$/, "")}${"}".repeat(deficits[index])}` : chunk);
...
function splitToolCallArray(text, start) {
	const chunks = [];
	let current = "";
	let inString = false;
	let escape = false;
	...
	async function* streamImpl(options) {
		...
		try {
			...
			if (tail.rejected) {
				...
				logger?.warn?.(`deepseek-web: 工具调用${tail.rejected.mode === "xml" ? "（XML）" : ""}解析失败[${reason}${reason === "unbalanced" ? "：多半是流被服务端上限截断" : "：结构不符"}]，已丢弃 ${tail.rejected.raw.length} 字符（将自动重试；完整原文见 ~/.dsh/deepseek-web/rejected.jsonl）：` + tail.rejected.raw.slice(0, 2e3));
			}
		} catch (error) {
			if (error instanceof AdapterLlmError) throw error;
...
async function openExternalLogin() {
	if (!electronAvailable()) return {
		ok: false,
		url: LOGIN_URL,
		message: "当前不是 Electron 桌面端：请手动在浏览器打开 chat.deepseek.com"
	};
	try {
...
async function logout() {
	...
	lastResult = {
		ok: true,
		message: cleared ? "已退出登录：本地凭证与浏览器登录态都已清除" : "已退出登录：本地凭证已清除（浏览器登录态未能清理——非 Electron 环境或清理失败，登录窗口可能仍是旧账号，请手动退出网页端）",
		at: (/* @__PURE__ */ new Date()).toISOString()
	};
	return cleared;
}
