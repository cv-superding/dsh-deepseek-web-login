	...
	async function* streamImpl(options) {
		...
		try {
			...
			for (;;) {
				...
				if (!(allowsAutoContinue(options?.purpose) && roundError === void 0 && deps.config.autoContinue !== false && rounds < maxRounds && toolCallCount === 0 && !options?.signal?.aborted && partial.length > 0 && roundChars > 0 && (midSentence || cutByServer))) break;
				rounds += 1;
			...
			try {
				...
				*/
				...
				if (req.method === "POST" && route === "/login/relogin") {
					...
					beginRelogin(id);
					logger.info?.(`deepseek-web: 准备重新登录「${target.label || target.id}」（不清理浏览器登录态，能复用就直接复用）—— 捕获后原地更新这条记录，不新增、也不切换当前账号`);
					sendJson(res, 200, {
						ok: true,
						targetId: id,
