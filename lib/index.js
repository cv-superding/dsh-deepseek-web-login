...
var ToolCallStreamFilter = class {
	...
	flush() {
		...
		if (this.capture) {
			...
			else if (captured.mode === "json") this.abandoned ??= {
				raw: captured.buffer,
				mode: "json"
			};
	...
	async function* streamImpl(options) {
		...
		try {
			...
			if (tail.rejected) {
				...
				logger?.warn?.(`deepseek-web: 工具调用${tail.rejected.mode === "xml" ? "（XML）" : ""}解析失败，已丢弃 ${tail.rejected.raw.length} 字符：${tail.rejected.raw.slice(0, 400)}`);
			}
		} catch (error) {
			if (error instanceof AdapterLlmError) throw error;
		...
		if (rejectedProtocol && hasVisibleText) {
			...
			yield {
				type: "text-delta",
				index: block.index,
				text: UNPARSED_TOOL_CALL_NOTICE
			};
		}
		if (!(outputChars > 0)) {
			yield {
