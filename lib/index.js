		...
		try {
			for await (const event of streamWebCompletion(auth, {
				...
				if (event.kind === "error") {
					...
					if (event.code === "RATE_LIMIT") throw new AdapterLlmError(`DeepSeek 网页端同一账号同时只能生成一条消息（另一个窗口/标签页正在用同一账号生成）。这一步会自动重试；若两个窗口都要用网页模型，建议其中一个换 provider 或换账号。`, "RATE_LIMIT", { ...event.retryAfterMs !== void 0 ? { providerRetryAfterMs: event.retryAfterMs } : {} });
