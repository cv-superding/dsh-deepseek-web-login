				...
				const createMsgNode = () => {
					...
					return {
						...
						set textContent(value) {
							...
							node.className = /失败|错误|无法|不对/.test(text) ? "dsw-msg err" : /^(✅|已切换|已移除|已捕获|已保存|已原地)/.test(text) ? "dsw-msg ok" : "dsw-hint dsw-gate-msg";
						}
					};
				};
				...
				*/
				...
				const accountRow = (item) => {
					...
					if (item.lastVerifyError) {
						...
						fix.append(el("span", "dsw-hint", `⚠️ ${relTime(item.lastVerifyError.at)}校验失败：${item.lastVerifyError.message}（凭证多半已失效；重新登录会原地更新它，不改变当前账号）`));
