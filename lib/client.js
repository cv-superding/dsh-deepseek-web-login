...
`;
		...
		function Panel() {
			...
			(0, react.useEffect)(() => {
				...
				const importInput = el("input", "dsw-input");
				importInput.placeholder = "要导入的备份文件路径";
				importInput.style.maxWidth = "320px";
				const importBtn = el("button", "dsw-btn ghost", "导入");
				...
				exportBtn.addEventListener("click", () => {
					(async () => {
						...
						try {
							...
							accountsMsg.textContent = result?.ok ? `已导出 ${result.count} 个账号到：${result.path}（含明文凭证，请妥善保管）` : `导出失败：${result?.error ?? "未知原因"}`;
						} catch (error) {
				...
				importBtn.addEventListener("click", () => {
					(async () => {
						...
						try {
							...
							accountsMsg.textContent = result?.ok ? `导入完成：新增 ${result.imported} / 更新 ${result.updated} / 跳过 ${result.skipped}` : `导入失败：${result?.error ?? "未知原因"}`;
							if (result?.ok) {
								importInput.value = "";
								await loadAccounts();
							}
				...
				const applyGate = (g) => {
					...
					cleanupHint.textContent = mode === "keep" ? "不删：请求最少，但网页端会留下临时会话记录。" : mode === "immediate" ? "立即：调用结束后 1.5 秒删掉（老行为，每轮多一个删除请求）。" : `延迟：攒够 ${g.cleanup?.batchSize ?? 8} 个、或 ${Math.round((g.cleanup?.delayMs ?? 9e4) / 1e3)} 秒后集中清理，并优先用一个请求批量删 —— 减少「每轮建一个立刻删一个」的机器特征。`;
				};
				const saveGate = async (patch) => {
					...
					try {
						...
						if (result?.ok) {
							applyGate(result);
							gateMsg.textContent = `已生效：${result.allowConcurrent ? "允许并发（不推荐）" : "串行"} · 间隔 ${result.minRequestIntervalMs}~${result.maxRequestIntervalMs}ms · 清理 ${result.sessionCleanup ?? result.cleanup?.mode ?? "-"}` + (result.persisted === false ? ` ⚠️ ${result.warning ?? "未能写入配置"}` : "（已写入配置，重启后仍生效）");
