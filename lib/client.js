...
`;
		...
		function Panel() {
			...
			(0, react.useEffect)(() => {
				...
				const intervalRange = el("input", "dsw-range");
				...
				gateCard.append(el("p", "dsw-hint", "间隔按「上一次请求结束」起算，所以长回答之后不会白等 —— 它只在密集连发时起作用。"));
				...
				/** 渲染宿主返回的节流设置（含可选档位）。 */
				const applyGate = (g) => {
					const interval = Number(g?.minRequestIntervalMs ?? 3e3);
					concInput.checked = !!g?.allowConcurrent;
					intervalRange.max = String(g?.maxIntervalMs ?? 3e4);
					...
					for (const ms of g?.presets ?? [
						1500,
						3e3,
						8e3
					]) {
						const preset = el("button", "dsw-btn ghost dsw-preset", ms === (g?.defaultIntervalMs ?? 3e3) ? `${ms}ms（推荐）` : `${ms}ms`);
				...
				const saveGate = async (patch) => {
					...
					try {
						...
						if (result?.ok) {
							applyGate(result);
							gateMsg.textContent = `已生效：${result.allowConcurrent ? "允许并发（不推荐）" : "串行"} · 间隔 ${result.minRequestIntervalMs}ms` + (result.persisted === false ? ` ⚠️ ${result.warning ?? "未能写入配置"}` : "（已写入配置，重启后仍生效）");
