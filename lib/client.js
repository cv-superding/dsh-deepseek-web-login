		...
		*/
		function pickJsonFile(doc = document) {
			return new Promise((resolve) => {
				...
				let settled = false;
				const timer = setTimeout(() => finish(void 0), 6e5);
				const finish = (file) => {
					if (settled) return;
					settled = true;
					clearTimeout(timer);
			...
			(0, react.useEffect)(() => {
				...
				*/
				...
				const paintLimit = () => {
					...
					limitRow.textContent = text === "已解除" ? `账号级限制已解除（${shortTime(activeLimitUntilMs)}）。` : `⚠️ 账号级限制：${text}（${shortTime(activeLimitUntilMs)} 解除，期间生成会被拒）`;
				};
				...
				*/
				...
				const ctxRange = el("input", "dsw-range");
				ctxRange.type = "range";
				ctxRange.min = "0";
				ctxRange.step = "1";
				...
				const formatCtxWindow = (tokens) => tokens >= 1048576 ? `${(tokens / 1048576).toFixed(0)}M` : `${Math.round(tokens / 1024)}K`;
				/** 把任意值吸附到最近的档位（配置里可能是旧值或手改过的值）。 */
				const nearestCtxIndex = (value) => {
					let best = 0;
				...
				const currentCtxTokens = () => ctxOptions[Number(ctxRange.value)] ?? ctxOptions[ctxOptions.length - 1];
				const paintContextWindow = () => {
					const tokens = currentCtxTokens();
					ctxValue.textContent = formatCtxWindow(tokens);
					ctxHint.textContent = tokens >= 1048576 ? "当前按 DeepSeek 标称的 1M 声明 —— DSH 会认为「还装得下」，尽量不压缩历史。如果你的任务都很短，可以调小以省下每轮重发的体量。" : `声明 ${formatCtxWindow(tokens)}：DSH 会更早压缩/截断历史，每轮重发的转写因此更短。这只是「声明值」，不改模型真实能力；调小只是让 DSH 早点动手。想彻底压住单次体量，上面的「prompt 上限」也要一起调。`;
				};
				ctxRange.addEventListener("input", paintContextWindow);
				ctxRange.addEventListener("change", () => {
					paintContextWindow();
					saveGate({ contextWindow: currentCtxTokens() });
				});
				const swRow = el("div", "dsw-gate-row");
				swRow.append(el("span", "dsw-gate-label", "自动换号"));
				const swRange = el("input", "dsw-range");
				swRange.type = "range";
				swRange.min = "0";
				swRange.max = "120";
				swRange.step = "1";
				...
				const currentSwitchMinutes = () => Math.max(0, Math.round(Number(swRange.value) || 0));
				const paintAutoSwitch = () => {
					const minutes = currentSwitchMinutes();
					swValue.textContent = minutes === 0 ? "关闭" : `${minutes} 分钟`;
					swHint.textContent = minutes === 0 ? "关闭时不会自动换号 —— 当前账号一直用到你手动切换为止。" : `每 ${minutes} 分钟换到账号库里的下一个可用账号（失效或正在受限的会跳过；可用的不足两个就不换）。换号会让投喂链断掉：下一轮要全量重发，历史图也要重新上传 —— 间隔越短，这个代价出现得越频繁。`;
				};
				swRange.addEventListener("input", paintAutoSwitch);
				swRange.addEventListener("change", () => {
					paintAutoSwitch();
					saveGate({ autoSwitchMinutes: currentSwitchMinutes() });
				});
				...
				const renderLedger = (data) => {
					...
					const byAccount = Array.isArray(data.byAccount) ? data.byAccount : [];
					if (byAccount.length > 0) {
						...
						push("按账号", byAccount.sort((a, b) => (Number(b?.calls) || 0) - (Number(a?.calls) || 0)).map((row) => `${nameOf(row.accountId)} ${row.calls} 次（失败 ${row.failed}）`).join(" · "));
					}
					const hourly = Array.isArray(data.hourly) ? data.hourly : [];
					const failedHours = /* @__PURE__ */ new Set();
					(Array.isArray(data.hourlyFailed) ? data.hourlyFailed : []).forEach((count, index) => {
				...
				*/
				...
				const applyGate = (g) => {
					...
					if (Array.isArray(g.contextWindowOptions) && g.contextWindowOptions.length > 0) {
						const parsedOptions = g.contextWindowOptions.map((value) => Number(value)).filter((value) => Number.isFinite(value) && value > 0);
						if (parsedOptions.length > 0) ctxOptions = parsedOptions;
					}
					ctxRange.min = "0";
					ctxRange.max = String(Math.max(0, ctxOptions.length - 1));
					ctxRange.value = String(nearestCtxIndex(Number(g.contextWindow ?? g.contextWindowDefault ?? ctxOptions[ctxOptions.length - 1])));
					paintContextWindow();
					const swBounds = g.autoSwitchBounds ?? {
						min: 0,
						max: 120
					};
