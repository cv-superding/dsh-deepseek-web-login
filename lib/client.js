...
`;
		...
		function Panel() {
			...
			(0, react.useEffect)(() => {
				...
				const logoutBtn = el("button", "dsw-btn danger", "退出登录");
				...
				const applyStatus = (status) => {
					...
					for (const model of status.models) {
						...
						const ctxLabel = model.contextWindow >= 1e6 ? `${(model.contextWindow / 1048576).toFixed(1)}M` : `${Math.round(model.contextWindow / 1024)}K`;
						item.append(el("div", "id", `${model.id} · thinking ${model.thinking ? "开" : "关"} · 上下文 ${ctxLabel} token`));
						item.append(el("div", "id", model.description));
						modelsList.append(item);
					}
					modelsHint.textContent = `两条是同一个「快速模式」的思考开关两档预设（也可在模型选择器的推理强度里切换）。图片输入直接可用（走网页端文件上传通道）。每次调用会新建并删除临时会话；prompt 字符上限 ${status.config?.maxPromptChars ?? 0}。`;
				};
