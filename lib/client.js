...
`;
		...
		function Panel() {
			...
			(0, react.useEffect)(() => {
				...
				accountsCard.append(el("p", "dsw-hint", "「登录新账号」会先清掉上次的浏览器登录态（库里已有的账号不受影响），登录后新账号只入库、不切换当前账号 ——加完在列表里点「切换」即可使用。导出/导入会弹系统对话框，自己选位置和文件。"));
				...
				const saveGate = async (patch) => {
					...
					try {
						...
						if (result?.ok) {
							applyGate(result);
							gateMsg.textContent = `已生效：${result.allowConcurrent ? "允许并发（不推荐）" : "串行"} · 间隔 ${result.minRequestIntervalMs}~${result.maxRequestIntervalMs}ms · 清理 ${result.sessionCleanup ?? result.cleanup?.mode ?? "-"}` + (result.cleanup?.batchRange ? `（阈值 ${result.cleanup.batchRange.min}~${result.cleanup.batchRange.max} 个 · 等待 ${Math.round((result.cleanup.delayRange?.min ?? 0) / 1e3)}~${Math.round((result.cleanup.delayRange?.max ?? 0) / 1e3)}s · ` + (result.cleanup.gapRange ? `删除间隔 ${(result.cleanup.gapRange.min / 1e3).toFixed(1)}~${(result.cleanup.gapRange.max / 1e3).toFixed(1)}s` : "删除间隔关") + "）" : "") + (result.persisted === false ? ` ⚠️ ${result.warning ?? "未能写入配置"}` : "（已写入配置，重启后仍生效）");
