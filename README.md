...
| `maxRefImages` | `24` | 一次请求最多带多少张历史图片（`ref_file_ids` 的长度）。网页端对这一批有上限（实测 40~52），超了整轮都会被拒、且**该会话此后每轮都失败**，所以按时间只带最近的 N 张；被略过的图在 prompt 里标成 `[earlier image omitted]`。`0` = 不限制（**不推荐**） |
