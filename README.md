...
| `minRequestIntervalMs` | **`3000`** | 两次网页端调用之间的最小间隔（毫秒），按上一次**结束**时刻计算。`0` = 关闭 |
...
| 场景 | `minRequestIntervalMs` | `allowConcurrent` |
|---|---|---|
| **推荐（默认）** | `3000` | `false` |
| 追速度、只跑短任务 | `1500` | `false` |
| 已被限流过 / 高密度自动化（多步骤工具调用） | `8000` | `false` |
| 完全关闭节流（**不建议**，会恢复并发重叠） | `0` | `false` |
| 实验：还原 DSH 原生并发 | 任意 | `true` ⚠️ |

> 间隔按「上一次调用**结束**」起算，所以长回答（几十秒）之后不会额外白等 —— 它只在
> 「结束 → 下一个开始」这段真正密集的空隙里起作用。
> 配置写在插件 entry config 里，改完**重启 DSH** 生效；设置页的「登录状态」卡会显示当前生效值。
